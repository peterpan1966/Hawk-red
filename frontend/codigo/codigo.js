...
//pedro agrego el get todos para pronto
const cliente = document.querySelector('.cliente');
const centro = document.querySelector('.centro');  //estaba .centro
const entidad = document.querySelector('.entidad');
const xlsx = document.querySelector('.xlsx');
...
fetch(`http://${API_URL}/fcdn/trazabilidad`,{
    method:'GET'
}).then(res=>res.json()).then(res=>options(res));
...
for (const iterator of e.ent) {
    const option = document.createElement('OPTION');
    option.classList.add('options');
option.value = iterator.ent_id;
option.innerHTML = iterator.ent_nombre;
// console.log(option)
entidad.appendChild(option);
}
//agregue clientes
for (const iterator of e.clientes) {
  const option = document.createElement('OPTION');
  option.classList.add('options');
option.value = iterator.ClienteID;
option.innerHTML = iterator.RazonSocial;
// console.log(option)
